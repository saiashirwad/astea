...
const asApplicationFailure = (planId: string, cause: unknown, rolledBack = false): ApplicationFailure =>
...
const safeTarget = (
  ...
  projectId: string,
  fileName: string,
): Effect.Effect<string, ApplicationFailure, FileSystem.FileSystem | Path.Path> => Effect.gen(function*() {
  const path = yield* Path.Path
  const fs = yield* FileSystem.FileSystem
  const project = plan.projects.find((candidate) => candidate.id === projectId)
  if (project === undefined || !isProjectRelativePath(fileName) || !isProjectRelativePath(project.configFileName)) {
    return yield* asApplicationFailure(plan.planId, `Unsafe or unknown project path: ${projectId}:${fileName}`)
  }
  ...
  if (!isContained(path, root, projectRoot) || !isContained(path, projectRoot, target) || target === projectRoot) {
    return yield* asApplicationFailure(plan.planId, `Path escapes project: ${projectId}:${fileName}`)
  }

  const realProjectRoot = yield* fs.realPath(projectRoot).pipe(
    Effect.mapError((cause) => asApplicationFailure(plan.planId, cause)),
  )
  const realWorkspaceRoot = yield* fs.realPath(root).pipe(
    Effect.mapError((cause) => asApplicationFailure(plan.planId, cause)),
  )
  if (!isContained(path, realWorkspaceRoot, realProjectRoot)) {
    return yield* asApplicationFailure(plan.planId, `Project root escapes workspace through symlink: ${projectId}`)
  }
  let existingParent = path.dirname(target)
  while (!(yield* fs.exists(existingParent).pipe(
    Effect.mapError((cause) => asApplicationFailure(plan.planId, cause)),
  ))) {
    if (existingParent === projectRoot) break
    const parent = path.dirname(existingParent)
    if (parent === existingParent || !isContained(path, projectRoot, parent)) {
      return yield* asApplicationFailure(plan.planId, `Path escapes project through parent: ${fileName}`)
    }
    existingParent = parent
  }
  const realParent = yield* fs.realPath(existingParent).pipe(
    Effect.mapError((cause) => asApplicationFailure(plan.planId, cause)),
  )
  if (!isContained(path, realProjectRoot, realParent)) {
    return yield* asApplicationFailure(plan.planId, `Path escapes project through symlink: ${fileName}`)
  }
  ...
  if (targetExists) {
    ...
    if (!isContained(path, realProjectRoot, realTarget)) {
      return yield* asApplicationFailure(plan.planId, `Target escapes project through symlink: ${fileName}`)
    }
  }
  return target
})
  ...
  Effect.gen(function*() {
    ...
    if (exists) {
      const current = yield* fs.readFileString(target).pipe(
        Effect.mapError(() => new StalePlanError({
          planId: plan.planId,
          projectId: file.projectId,
          fileName: file.fileName,
        })),
    ...
    apply: Effect.fn("PlanApplication.apply")(function*(verified: VerifiedPlan) {
      ...
      return yield* Effect.gen(function*() {
        ...
        for (const source of plan.sources) {
          ...
          const current = yield* fs.readFileString(target).pipe(Effect.mapError(() => new StalePlanError({
            planId: plan.planId,
            projectId: source.projectId,
            fileName: source.fileName,
          })))
          if (textHash(current) !== source.hash) {
            return yield* new StalePlanError({ planId: plan.planId, projectId: source.projectId, fileName: source.fileName })
          }
        }
        ...
        const stageExit = yield* Effect.gen(function*() {
          ...
          Effect.mapError((error) => error instanceof StalePlanError
            ? error
            : asApplicationFailure(plan.planId, error)),
          Effect.result,
        )
            ...
            for (const item of [...applied].reverse()) {
              ...
              if (!item.file.before.exists) {
                ...
                yield* Effect.gen(function*() {
                  yield* fs.writeFileString(rollbackTemporary, item.file.before.text ?? "", { flag: "wx" })
