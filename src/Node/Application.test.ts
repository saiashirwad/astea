...
const exists = (fileName: string): Effect.Effect<boolean> =>
  Effect.promise(() => Fs.stat(fileName).then(() => true, () => false))
...
describe("Node application transactions", () => {
  effect("rejects a create-target race without overwriting the raced file", () =>
    withFixture((root, app) =>
      Effect.gen(function*() {
        ...
        expect(yield* Effect.promise(() => Fs.readFile(target, "utf8"))).toBe("created by another process\n")
      }),
    ),
  )
  ...
  effect("uses a no-clobber commit when a create target appears after the final check", () =>
    withFixture((root, app) =>
      Effect.gen(function*() {
        ...
        const result = yield* Application.apply(verified).pipe(Effect.provide(testLayer), Effect.result)
        expect(result._tag).toBe("Failure")
      ...
      Effect.gen(function*() {
        ...
        const recipe = Recipe.define("empty-file-lifecycle", {
          ...
          run: () => Effect.gen(function*() {
            ...
            const move = yield* Draft.files.move(project, "src/move-empty.ts", "src/moved-empty.ts")
        ...
        expect(yield* Effect.promise(() => Fs.readFile(Path.join(root, "src/created-empty.ts"), "utf8"))).toBe("")
        ...
        expect(yield* Effect.promise(() => Fs.readFile(Path.join(root, "src/moved-empty.ts"), "utf8"))).toBe("")
  ...
  effect("rolls back earlier files in reverse order and removes all staged temporaries", () =>
    withFixture((root, app) =>
      Effect.gen(function*() {
        ...
        const originalConsumer = yield* Effect.promise(() => Fs.readFile(Path.join(root, "src/consumer.ts"), "utf8"))
        const originalReexport = yield* Effect.promise(() => Fs.readFile(Path.join(root, "src/reexport-consumer.ts"), "utf8"))
        const realFs = yield* FileSystem.FileSystem
        let renameCount = 0
        ...
        if (result._tag === "Failure") {
          ...
          if (result.failure._tag === "ApplicationFailure") expect(result.failure.rolledBack).toBe(true)
        }
        expect(yield* Effect.promise(() => Fs.readFile(Path.join(root, "src/consumer.ts"), "utf8"))).toBe(originalConsumer)
        expect(yield* Effect.promise(() => Fs.readFile(Path.join(root, "src/reexport-consumer.ts"), "utf8"))).toBe(originalReexport)
  ...
  effect("rejects a symlinked project subdirectory that escapes the workspace", () =>
    withFixture((root, app) =>
      Effect.gen(function*() {
        const outside = yield* Effect.promise(() => Fs.mkdtemp(Path.join(Path.dirname(root), "safemods-outside-")))
        ...
        const recipe = Recipe.define("symlink-escape", {
          ...
          run: () => Effect.gen(function*() {
            const snapshot = yield* WorkspaceSnapshot
            const project = yield* snapshot.project(app)
            return yield* Draft.files.create(project, "src/escape/outside.ts", "export const escaped = true;\n")
          }),
        })
        const plan = yield* Recipe.run(recipe, undefined)
        const verified = yield* Verification.verify(plan, recipe, undefined)
