...
describe("declarative transformations API (@effect/vitest)", () => {
  describe("in-memory snapshot transitions", () => {
    ...
    effect("keeps file lifecycle state coherent across piped stages", () =>
      withFixture((_, app) =>
        Effect.gen(function*() {
          const workspace = yield* Workspace
          yield* workspace.withSnapshot({}, Effect.gen(function*() {
            ...
            const createDraft = yield* Draft.files.create(project, "src/generated.ts", createdText)
            const createdOverlay = yield* Overlay.materialize(snapshot, [{
              projectId: app.id,
            ...
            expect(createdOverlay[Path.resolve(project.root, "src/generated.ts")]).toContain("value = 2")

            const source = yield* project.sourceText("src/library.ts")
            const moveDraft = yield* Draft.files.move(project, "src/library.ts", "src/shared/core.ts")
            const movedOverlay = yield* Overlay.materialize(snapshot, [{
              projectId: app.id,
            ...
            expect(movedOverlay[Path.resolve(project.root, "src/shared/core.ts")]).toContain("moved edit")

            const deleteDraft = yield* Draft.files.delete(project, "src/library.ts")
            const deletedOverlay = yield* Overlay.materialize(snapshot, [{
              projectId: app.id,
            ...
            const textEdit = Recipe.define("text-edit", {
              ...
              run: () => Effect.gen(function*() {
                ...
                return yield* Draft.replace(project, source, `${source.text}\n// text stage\n`)
              }),
            })
            const operationOnly = Recipe.define("operation-only", {
              version: "1.0.0",
              run: () => Draft.files.create(project, "src/operation-only.ts", "export const operationOnly = true;\n"),
            })
    ...
    effect("lets later piped recipes query created or moved files and makes delete terminal", () =>
      withFixture((_, app) =>
        Effect.gen(function*() {
          const create = Recipe.define("pipe-create", {
            ...
            run: () => Effect.gen(function*() {
              const snapshot = yield* WorkspaceSnapshot
              const project = yield* snapshot.project(app)
              return yield* Draft.files.create(project, "src/generated.ts", "export const generated = 1;\n")
            }),
          })
          ...
          const createOperation = createdPlan.fileOperations?.find((operation) => operation.kind === "create")
          expect(createOperation?.kind === "create" ? createOperation.content : "").toContain("generated = 2")
          ...
          const moveCreated = Recipe.define("pipe-move-created", {
            ...
            run: () => Effect.gen(function*() {
              const snapshot = yield* WorkspaceSnapshot
              const project = yield* snapshot.project(app)
              return yield* Draft.files.move(project, "src/generated.ts", "src/moved-generated.ts")
            }),
          })
          ...
          const createThenDelete = yield* Recipe.run(Recipe.pipe(create, deleteCreated), undefined)
          ...
          const editMoved = Recipe.define("pipe-edit-moved", {
            ...
            run: () => Effect.gen(function*() {
              ...
              return yield* Draft.replace(project, source, `// edited after move\n${source.text}`)
            }),
          })
          const movedPlan = yield* Recipe.run(Recipe.pipe(move, editMoved), undefined)
          const moveOperation = movedPlan.fileOperations?.find((operation) => operation.kind === "move")
          expect(moveOperation?.kind === "move" ? moveOperation.content : "").toContain("edited after move")
          expect(movedPlan.edits.some((edit) => edit.fileName === "src/moved-library.ts")).toBe(false)
          ...
          expect(deletedPlan.fileOperations?.some((operation) => operation.kind === "delete")).toBe(true)
          expect(deletedPlan.edits.some((candidate) => candidate.fileName === "src/library.ts")).toBe(false)
        })
      ),
      60_000,
    )
