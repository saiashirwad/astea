...
describe("Edit", () => {
  effect("sorts edits deterministically", () => Effect.gen(function*() {
    const normalized = yield* normalizeEdits([edit(4, 5, "E"), edit(1, 2, "B")])
    expect(normalized.map((item) => item.start)).toEqual([1, 4])
  }))

  effect("rejects insertions at the same position and overlapping replacements", () => Effect.gen(function*() {
  ...
  effect("guards expected source text", () => Effect.gen(function*() {
